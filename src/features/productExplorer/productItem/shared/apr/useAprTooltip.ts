...
const maBeetsRewardTooltipText =
  'This is the APR you will receive when a relic is fully matured.';

// Only include the hook types we currently support irt dynamic swap fees
export type SupportedHookType = Extract<
  GqlHookType,
  GqlHookType.MevTax | GqlHookType.StableSurge
>;
...
export function useAprTooltip({
  ...
  // Swap fees
  const swapFee = aprItems.find(
    (item) => item.type === GqlPoolAprItemType.SwapFee_24H
  );
  const swapFeesDisplayed = numberFormatter(
    swapFee ? swapFee.apr.toString() : '0'
  );

  // Dynamic wap fees (MEV Capture, StableSurge)
  const dynamicSwapFee = aprItems.find(
    (item) => item.type === GqlPoolAprItemType.DynamicSwapFee_24H
  );
  ...
  const votingApr = aprItems.find(
    (item) => item.type === GqlPoolAprItemType.Voting
  );
  const votingAprDisplayed = numberFormatter(
    votingApr ? votingApr.apr.toString() : '0'
  );

  const lockingApr = aprItems.find(
    (item) => item.type === GqlPoolAprItemType.Locking
  );
  const lockingAprDisplayed = numberFormatter(
    lockingApr ? lockingApr.apr.toString() : '0'
  );

  // Merkl incentives
  const merklIncentives = filterByType(aprItems, GqlPoolAprItemType.Merkl);
  const hasMerklIncentives = merklIncentives.length > 0;
  const merklIncentivesAprDisplayed =
    calculateSingleIncentivesAprDisplayed(merklIncentives);
  ...
  // Surplus incentives
  const surplusIncentives = filterByType(
    aprItems,
    GqlPoolAprItemType.Surplus_24H
  );
  const surplusIncentivesAprDisplayed =
    calculateSingleIncentivesAprDisplayed(surplusIncentives);
  ...
  const maBeetsReward = aprItems.find(
    (item) => item.type === GqlPoolAprItemType.MabeetsEmissions
  );
  ...
  const maxMaBeetsReward = aprItems.find(
    (item) =>
      item.type === GqlPoolAprItemType.StakingBoost &&
      item.id.match(/beets-apr-boost/) // TODO: or add prop in api to distinguish?
  );
  ...
  const maxMaBeetsVotingReward = aprItems.find(
    (item) =>
      item.type === GqlPoolAprItemType.StakingBoost &&
      item.id.match(/voting-apr-boost/) // TODO: or add prop in api to distinguish?
  );
  ...
  const extraBalAprDisplayed = hasVeBalBoost
    ? maxVeBalDisplayed.minus(totalBaseDisplayed)
    : bn(0);
