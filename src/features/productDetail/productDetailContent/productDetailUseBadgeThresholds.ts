...
export function useBadgeThresholds(productAddress: string, livePools: LivePoolsShape) {
  return useMemo(() => {
    let gold = 0, silver = 0, bronze = 0, prefix = 'UNKNOWN';
    const depos = livePools.factsheets.find((y) => y.poolId === productAddress)?.depositorBadges;
