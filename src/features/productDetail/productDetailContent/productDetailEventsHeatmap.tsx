...
export const ProductDetailEventsHeatmap: FC<EventsHeatmapProps> = memo(function EventsHeatmap({
  chartTheme,
  data,
  xDomain,
  yDomain,
  ...
  function itemStyler(params: any): AgHeatmapSeriesStyle | undefined {
    ...
    const t = Number.isFinite(v) && max > min ? Math.min(1, Math.max(0, (v - min) / (max - min))) : 0.5;
    ...
    const brighten = (css: string, amt = 0.1) => {
      try {
        ...
        const [nR, nG, nB] = [toSrgb(Rl + (1 - Rl) * tAmt), toSrgb(Gl + (1 - Gl) * tAmt), toSrgb(Bl + (1 - Bl) * tAmt)];
        return `rgb(${nR}, ${nG}, ${nB})`;
      } catch {
        return css;
      }
  ...
  useEffect(() => {
    ...
    const addrToName = (addr?: string) => addrNameMap[String(addr ?? '').toLowerCase()] ?? String(addr ?? '');
    ...
    const options: AgChartOptions = {
      ...
      theme: {
        ...
        overrides: {
          heatmap: {
            series: { colorRange: ['#f96103ff', '#599707ff'], strokeWidth: 0.4, strokeOpacity: 0.5 } as AgHeatmapSeriesStyle,
          },
        },
      },
      ...
      axes: [
        {
          ...
          type: 'category',
          tick: { size: 10 },
          label: { padding: 0, rotation: 45, fontSize: 8, formatter: ({ value }: any) => addrToName(String(value)) },
        },
        {
          position: 'bottom',
          type: 'category',
          line: { enabled: false },
          label: { fontSize: 8, padding: 0, formatter: ({ value }: any) => addrToName(String(value)) },
        },
      ],
      gradientLegend: {
        scale: {
