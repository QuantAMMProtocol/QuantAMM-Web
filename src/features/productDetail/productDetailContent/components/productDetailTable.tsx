...
export const ProductDetailTable: FC<ProductDetailTableProps> = memo(
  function ProductDetailTable({ simulationRunBreakdown, productId, isMobile }) {
    if (!isMobile) {
      return simulationRunBreakdown ? (
        <AnalysisSimplifiedBreakdownTable
          ...
          visibleMetrics={[
            ...(simulationRunBreakdown.simulationRunResultAnalysis?.return_analysis.map(
              (x) => x.metricName
            ) ?? []),
